...
def conv_pool_forward(x, w, b, conv_param, pool_param):
  """
  Convenience layer that performs a convolution, and a pool.
  ...
  """
  a, conv_cache = conv_forward_naive(x, w, b, conv_param)
  out, pool_cache = max_pool_forward_naive(a, pool_param)
...
def conv_pool_backward(dout, cache):
  """
  Backward pass for the conv-pool convenience layer
  """
  conv_cache, pool_cache = cache
  ds = max_pool_backward_naive(dout, pool_cache)
  dx, dw, db = conv_backward_naive(ds, conv_cache)
  return dx, dw, db
